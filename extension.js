...
*/
  ...
  Has: function(File) {
    ...
    for (Iter = 0; Iter < AutoFoldTracker.Files.length; Iter++)
      if (AutoFoldTracker.Files[Iter] == File.uri.fsPath) return true;
  ...
  Fold: function(File) {
    ...
    if (Config.foldwords) {
      AutoFoldTracker.PrintDebug(">> foldwords(" + Config.foldwords + ")");
      SpecificFolds = AutoFoldTracker.GetSpecificFolds(File, Config.foldwords);
    }

    AutoFoldTracker.PrintDebug(
      ">> foldLines(" + SpecificFolds.join(", ") + "): " + File.uri.fsPath
    );
  ...
  CommandFold: function() {
    ...
    if (Current) AutoFoldTracker.Fold(vscode.window.activeTextEditor.document);
  ...
  OnFileOpen: function(File) {
    ...
    AutoFoldTracker.Interval = setTimeout(function() {
      if (AutoFoldTracker.Has(File)) return;
  ...
  OnFileClose: function(File) {
    ...
    setTimeout(function() {
      ...
      for (Iter = 0; Iter < vscode.workspace.textDocuments.length; Iter++) {
        ...
        if (vscode.workspace.textDocuments[Iter].uri.fsPath == Filename) return;
      }
      ...
      // pull it out.
      for (Iter = 0; Iter < AutoFoldTracker.Files.length; Iter++)
        if (File.uri.fsPath != AutoFoldTracker.Files[Iter]) Files.push(AutoFoldTracker.Files[Iter]);
  ...
  GetSpecificFolds: function(File, FoldWords) {
    /*
	@argv TextDocument
	@return Int[]
	...
	*/
    ...
    let SpecificFolds = [];
    AutoFoldTracker.PrintDebug(">> Words: " + FoldWords);
    // attempt to find instances of vscode-fold$ in the file
    for (let i = 0; i < File.lineCount; i++) {
      for (let j = 0; j < FoldWords.length; j++) {
        if (RegExp(FoldWords[j].word).test(File.lineAt(i).text)) {
  ...
  GetLevel: function(File) {
    ...
    if (Result && Result.length == 2 && parseInt(Result[1]) > 0) Level = Result[1];

    ////////

    // fallback to configured values.

    if (Level == 0) Level = AutoFoldTracker.GetDefaultLevel(AutoFoldTracker.GetCurrentDocument());
    else AutoFoldTracker.PrintDebug("## using file vscode-fold: " + Level);
    ...
    if (Config.types) {
      ...
      for (Iter = 0; Iter < Config.types.length; Iter++) {
        ...
        if (ExtCurr.indexOf(ExtConf) == ExtCurr.length - ExtConf.length) {
          AutoFoldTracker.PrintDebug(
            "## using autofold.types[" + Config.types[Iter].ext + "]: " + Config.types[Iter].level
          );
  ...
  GetCurrentDocument: function() {
    ...
    if (vscode.window.activeTextEditor)
      if (vscode.window.activeTextEditor.document) return vscode.window.activeTextEditor.document;

    return null;
  },

  IsTypeConfigValid: function(TypeObject) {
    /*//
	@argv Object
	@return Bool
    ...
    if (typeof TypeObject != "object") return false;

    if (typeof TypeObject.ext != "string") return false;

    if (typeof TypeObject.level != "number") return false;
  ...
  PrintDebug: function(Msg) {
    ...
    if (AutoFoldTracker.Debug) console.log("[AF] " + Msg);
...
//*/
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("extension.autofold", AutoFoldTracker.CommandFold)
  );

  vscode.workspace.onDidOpenTextDocument(AutoFoldTracker.OnFileOpen, null, context);

  vscode.workspace.onDidCloseTextDocument(AutoFoldTracker.OnFileClose, null, context);
